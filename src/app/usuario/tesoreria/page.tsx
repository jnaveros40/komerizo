...
type Movimiento = {
  ...
  fecha_movimiento?: string; beneficiario_destino?: string; metodo_pago?: string
}
...
export default function UsuarioTesoreria() {
  ...
  return <div className="tesoreria-container">
    ...
    <div className="movimientos-section"><h2>Últimos Egresos Registrados</h2>
      {loading ? <div className="loading">Cargando movimientos financieros...</div> : movimientos.length === 0 ? <div className="empty-state">No hay egresos financieros registrados.</div> : <div className="table-responsive"><table className="movimientos-table"><thead><tr><th>Fecha y hora</th><th>Concepto</th><th>Beneficiario</th><th>Responsable</th><th>Monto</th><th>Soporte</th><th>Informe</th></tr></thead><tbody>{movimientos.map(mov => <tr key={mov.id}><td><div className="td-datetime"><span className="td-date">{mov.fecha}</span><span className="td-time">{mov.hora}</span></div></td><td><div className="td-concepto"><strong>{mov.descripcion}</strong>{mov.justificacion && <span className="td-justificacion">{mov.justificacion}</span>}</div></td><td>{mov.beneficiario_destino || '-'}</td><td><span className="td-rol">{mov.rol}</span></td><td className="td-monto gasto">-{currency(mov.cantidad)}</td><td>{mov.archivo_adjunto_url ? <a href={mov.archivo_adjunto_url} target="_blank" rel="noreferrer" className="btn-comprobante">📄 Ver</a> : mov.referencia_externa || 'N/A'}</td><td><button className="btn-comprobante" disabled={pendingRequest} onClick={() => setSelected(mov)}>Solicitar informe</button></td></tr>)}</tbody></table></div>}
